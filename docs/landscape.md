...
**On economics:** the two lanes are priced for different buyers, by design. Creative Production workflows are Operations-metered, and that price includes Adobe running the production line — managed orchestration, governance, and human-review surfaces. Parameter-level API calls bill as ordinary Firefly Services API consumption under your own contract, because you're bringing your own orchestration. Teams that already own their pipeline infrastructure (queues, retries, review) typically want the latter cost structure; teams that want Adobe to operate it want the former. Both are legitimate — choose per workload.
